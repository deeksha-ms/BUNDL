...
class transformer_lstm_nal(nn.Module):
    def __init__(self, cnn_dropout=0.0, gru_dropout=0.0, transformer_dropout=0.2, device='cpu', return_attn=False):
        super().__init__()
...
class TGCN_nal(nn.Module):
    ...
    def __init__(self, connections_fn="", device = 'cpu'):
        super().__init__()
        ...
        self.noise_layer_simple = nn.Linear(2, 2, bias = False)
        self.noise_layer_simple.weight.data =  torch.eye(2).double().to(device)
        self.noise_layer_complex = nn.Linear(512, 4, bias = False)
    ...
    def forward_pass(self, x):
        ...
        clean_out = self.linear3(h_m).reshape(-1, 2)                         # (B, T, 2)

        # Reshaping
        #h = h.transpose(1, 2)                       # (B, T, C, 2)    
        
        h_m = h_m.reshape(B*Nsz*T, 512)        
        bias = self.noise_layer_simple(self.eye)
        data  = self.noise_layer_complex(h_m).reshape(-1, 2, 2)
...
class CNN_BLSTM_nal(nn.Module):
    ...
    def __init__(self, nchns=19, device = 'cpu'):
        super(CNN_BLSTM_nal, self).__init__()
        ...
        self.noise_layer_simple = nn.Linear(2, 2, bias = False)
        self.noise_layer_simple.weight.data =  torch.eye(2).double().to(device)
        self.noise_layer_complex = nn.Linear(40, 4, bias = False)
    ...
    def forward(self, x):
        ...
        bias = self.noise_layer_simple(self.eye)
        data  = self.noise_layer_complex(rout).reshape(-1, 2, 2)
        ...
        return clean_out.reshape(b,Nsz,T, -1), noise_out.reshape(b,Nsz,T, -1), None
