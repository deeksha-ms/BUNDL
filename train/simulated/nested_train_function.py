...
def nested_train_function(data_root, modelname='txlstm', cvfold=1,
        ...
        test_pts =   all_pts[(ind-1)*valsize : ind*valsize]
        train_pts = [pt for pt in all_pts if pt not in val_pts][:96]
        val_pts = [pt for pt in all_pts if pt not in val_pts][96:]
        ...
        savename = modelname+'_'+traintype+'_cv'+str(cvfold)+'_'+str(lr)+'_'+str(version)+'_'+str(noise_type)+ '_nested'
